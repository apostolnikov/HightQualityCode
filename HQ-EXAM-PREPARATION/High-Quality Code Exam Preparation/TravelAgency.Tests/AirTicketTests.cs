namespace TravelAgency.Tests
{
    ...
    {
        private ITicketCatalog catalog;  // chast ot isntanciraneto po-dolu,  oburni vnimanie che e INTERFACE

        [TestInitialize]
        public void InitializeTest()
        {
            Thread.CurrentThread.CurrentCulture = CultureInfo.InvariantCulture;
            catalog = new TicketCatalog();                // iniciqlizirame, suzdavame instanciq na TicketCatalog.cs i polzvame metodite na klasa 
                                                         // chrez this.catalog.METHOD       
        }

        [TestMethod]
        public void Test_AddAirTicket_ShouldAddTheTicket()
        {
            string message = this.catalog.AddAirTicket(     // prisvoqvame na promenliva ot tip string, suobshtenieto koeto trqbva da vurne TicketCatalog.cs sled dobavqne na bilet
                flightNumber: "FX215",
                from: "Sofia",
                to: "Athens",
        ...
        {
            this.catalog.AddAirTicket(                 // dobavqme edin
                flightNumber: "FX215",
                from: "Sofia",
                to: "Athens",
                airline: "Bulgaria Air",
                dateTime: new DateTime(2015, 1, 17, 12, 20, 0),
                price: 200);
            string message = this.catalog.AddAirTicket(   // advame vtori ednakuv, zapazvame suobshtenieto ot vtoriq v string message
                flightNumber: "FX215",
                from: "Sofia",
                to: "Athens",
                airline: "Bulgaria Air",
